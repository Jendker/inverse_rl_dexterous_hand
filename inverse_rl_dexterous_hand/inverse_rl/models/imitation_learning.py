...
from inverse_rl_dexterous_hand.inverse_rl.models.architectures import feedforward_energy, relu_net
from inverse_rl_dexterous_hand.inverse_rl.utils.general import TrainingIterator
from inverse_rl_dexterous_hand.inverse_rl.utils.hyperparametrized import Hyperparametrized
from inverse_rl_dexterous_hand.inverse_rl.utils.math_utils import gauss_log_pdf, categorical_log_pdf
import inverse_rl_dexterous_hand.inverse_rl.utils.tensor_utils as tensor_utils
...
class TensorFlowIL:
    ...
    def save_checkpoint(path, iteration):
        ...
        saver.save(tf.get_default_session(), os.path.join(path, 'discriminator_{}.ckpt'.format(iteration)))

    @staticmethod
    def load_checkpoint(path=None, iteration=None, full_path=None):
        saver = tf.train.Saver()
        sess = tf.get_default_session()
        if full_path is None:
            full_path = os.path.join(path, 'discriminator_{}.ckpt'.format(iteration))
        if sess is not None:
            try:
            ...
            raise ValueError('TF default session is None')

    @staticmethod
    def get_full_checkpoint_path(path, iteration):
        return os.path.join(path, 'discriminator_{}.ckpt'.format(iteration))
...
class ImitationLearning(object, metaclass=Hyperparametrized):
    ...
    @staticmethod
    def sample_batch(*args, batch_size=32, path_index_batches=None):
        N = args[0].shape[0]
        if path_index_batches is not None:
            batch_idxs = path_index_batches
        else:
...
class GAIL(SingleTimestepIRL, TensorFlowIL):
    """
    ...
    """
    def __init__(self, env, expert_trajs=None,
...
class AIRLStateAction(SingleTimestepIRL, TensorFlowIL):
    """
    This version consumes single timesteps. 
    """
    def __init__(self, env, expert_trajs=None,
...
class GAN_GCL(TrajectoryIRL, TensorFlowIL):
    """
    ...
    def __init__(self, env, expert_trajs=None,
        ...
        with tf.variable_scope(name) as vs:
            ...
            if discount >= 1.0:
                ...
                log_p_tau = self.discounted_reduce_sum(-self.energy, discount=discount, axis=1)
